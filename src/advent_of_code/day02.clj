...
(defn input->spreadsheet [input n-cols]
  (->> (str/split input #"\s")
       (map #(Integer/parseInt %))
       (partition n-cols)
       (mapv vec)))

(def spreadsheet (input->spreadsheet input 16))

(defn part-one [spreadsheet]
  (->> spreadsheet
       (map (fn [vs]
              (let [max (apply max vs)
                    min (apply min vs)]
                (Math/abs (- max min)))))
       (reduce +)))

(defn evenly-divisible-pairs [row]
  (->> row
       (mapcat (fn [x]
                 (let [candidate-xs (filter #(>= x %) row)]
                   (keep #(when (zero? (mod x %))
                            [x %])
                         candidate-xs))))
       (filter (fn [[l r]]
                 (not= l r)))
...
(defn part-two [spreadsheet]
  (->> (map evenly-divisible-pairs spreadsheet)
       (map (fn [[l r]]
              (/ l r)))
       (reduce +)))
...
(comment
  ...
  (part-two (input->spreadsheet input2 4))
